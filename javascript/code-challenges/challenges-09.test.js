...
const hasChildrenEntries = (arr, character) => {
  const foundCharacter = arr.find(char => char.name === character);
  return foundCharacter && Object.entries(foundCharacter).some(([key, value]) => key === 'children' && value.length > 0);
};
...
const houseSize = (arr) => {
  return arr.map(character => {
    const members = character.spouse ? character.children ? character.children.length + 2 : 2 : character.children ? character.children.length + 1 : 1;
...
const houseSurvivors = (arr) => {
  return arr.map(character => {
    const members = character.spouse && !deceasedSpouses.includes(character.spouse) ? character.children ? character.children.length + 2 : 2 : character.children ? character.children.length + 1 : 1;
    return { house: character.house, members };
  });
};
