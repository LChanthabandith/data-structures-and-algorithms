...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, cur) => acc + (cur.children ? cur.children.length : 0), 0);
};
...
const calculateAverage = (arr) => {
  let { sum, count } = arr.reduce((acc, cur) => ({sum: acc.sum + cur, count: acc.count + 1}), {sum: 0, count: 0});
  return sum / count;
};
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((acc, cur) => {
    acc[cur.stat.name] = cur.baseStat;
    return acc;
  }, {});
};
...
const extractChildren = (arr) => {
  return arr
    ...
    .reduce((acc, cur) => acc.concat(cur.children ? cur.children : []), []);
};
